...
class OrderService {
  async create(userId, { paymentMethod, notes, transactionRef }) {
        ...
        { item: ci.item._id },
        {
          ...
          $push: {
            history: {
              action: "deduct",
              qty: ci.quantity,
              note: `Order #${String(order._id).slice(-8)}`,
              date: new Date(),
            },
          },
        },
    ...
    if (cart.walletDeduction > 0) {
      ...
      if (wallet) {
        wallet.balance = parseFloat(
          (wallet.balance - cart.walletDeduction).toFixed(2),
        );
        await wallet.save();
    ...
    await Notification.create({
      user: userId,
      title: "Order Placed ✅",
      message: `Your order #${String(order._id).slice(-8).toUpperCase()} has been placed`,
      type: "order",
    ...
    if (user)
      sendEmail({
        ...
        html: orderConfirmEmail(user.name, order),
      }).catch(() => {});
